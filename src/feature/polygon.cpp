...
polygon_t::polygon_t(shplib::opaque_object_t feature) : m_underlying(std::move(feature))
{
...
bool polygon_t::is_compatible(shape_type t) noexcept
{
    return t == shape_type::polygon || t == shape_type::polygon_z || t == shape_type::polygon_m;
}

coordinate_xyzm_t polygon_t::min_bound() const noexcept { return m_min_bound; }
coordinate_xyzm_t polygon_t::max_bound() const noexcept { return m_max_bound; }

bool polygon_t::has_z() const noexcept { return m_type == shape_type::polygon_z; }
bool polygon_t::has_m() const noexcept { return m_m_is_present; }

std::size_t polygon_t::num_parts() const noexcept { return m_parts; }
std::size_t polygon_t::num_points() const noexcept { return m_points; }

polyline_part_t polygon_t::at(std::size_t index) const
{
    if (index >= m_parts)
    {
