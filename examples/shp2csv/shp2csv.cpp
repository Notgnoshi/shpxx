...
int main(int argc, const char* argv[])
{
    ...
    case shape_type::polygon_z:
    case shape_type::multipoint_z:
    // ESRI spec Table 16: Z is mandatory for MultiPatch, M is optional.
    case shape_type::multipatch:
        // M is optional for Z and MultiPatch records (ESRI spec, asterisk on
        // M fields in Tables 13-16). Shapelib does not expose an "has M" flag
        // on SHPHandle or SHPObject -- when M is absent it still allocates
        // padfM and fills it with the spec's "no data" sentinel (< -1e38, p. 2).
        //
        // Heuristic: M is present iff the file-header M bounds disagree. This
        // false-negatives when M is present but all values are equal -- e.g.,
        // every record stores the same constant or every record stores the
        // NODATA sentinel. A stricter check would scan padfM[0] of each shape
        // for the NODATA sentinel, but that requires reading all records.
